...
if (!empty($_POST)) {
    ...
    try {
        ...
        if ($m2signature == $partnerSignature) {
            if ($errorCode == '0') {
                $result = '<div class="alert alert-success" role="alert">Capture Payment Success</div>';
            } else {
                $result = '<div class="alert alert-danger" role="alert">' . $message . '</div>';
            }
        } else {
            $result = '<div class="alert alert-danger" role="alert">This transaction could be hacked, please check your signature and returned signature</div>';
        }
    } catch (Exception $e) {
